...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    if self.activation is 'relu':
        ...
        hidden = np.log(1.0 + np.exp(-np.abs(z1))) + np.maximum(z1,0)
    ...
    for i in range(len(scores)):
        m = np.max(scores[i])
        softmax_scores.append(np.exp(scores[i] - m) / np.sum(np.exp(scores[i] - m)))

    data_loss = 0.
    for i in range(len(softmax_scores)):
        data_loss += -np.log(softmax_scores[i][y[i]] + np.finfo(float).eps)

    regularized_loss = np.linalg.norm(W1)**2 + np.linalg.norm(W2)**2

    loss = data_loss/N + 0.5 * reg * regularized_loss
    ###########################################################################
    #                            END OF YOUR CODE
    ###########################################################################
    ...
    # output layer
    dscore = [[softmax_scores[i][j]-1 if(y[i]==j) else softmax_scores[i][j] for j in range(C)] for i in range(N)]  # partial derivative of loss wrt. the logits (dL/dz)
    dW2 = np.dot(hidden.T, dscore) / N + reg * W2    # partial derivative of loss wrt. W2
    db2 = np.sum(dscore, axis=0) / N     # and so on...
    ...
    if self.activation is 'relu':
        ...
        dsoftplus = expit(z1)
        dz1 = [[dhidden[i][j] * dsoftplus[i][j] for j in range(len(dhidden[i]))] for i in range(len(dhidden))]
    elif self.activation is 'sigmoid':
        dsigmoid = hidden * (1 - hidden)
        dz1 = [[dhidden[i][j] * dsigmoid[i][j] for j in range(len(dhidden[i]))] for i in range(len(dhidden))]
    else:
        raise ValueError('Unknown activation type')
